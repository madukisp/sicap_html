document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const mes = getSelectedRadio('mes');
        const ano = getSelectedRadio('ano');

        if (!file) {
            showStatus('Selecione um arquivo Excel.', 'error');
            return;
        }
        // Validação HTML5 'required' já deve ter cuidado de user/pass/radios, mas checamos por segurança
        if (!user || !pass || !mes || !ano) {
            showStatus('Preencha todas as informações obrigatórias.', 'error');
            return;
        }
        ...
        try {
            ...
            if (response.ok && result.status === 'sucesso') {
                ...
                // Tenta salvar senha no navegador
                if (window.PasswordCredential) {
                    try {
                        const cred = new PasswordCredential({
                            id: user,
                            password: pass,
                            name: user, // Opcional, mas ajuda
                        });
