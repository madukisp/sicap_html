...
def processar_planilha(caminho_arquivo: str, usuario: str, senha: str, mes: str = None, ano: str = None) -> dict:
    ...
    try:
        ...
        # Busca ID no mapa (Estrutura: Ano -> Mes)
        mapa_anos = MAPAS.get("PrestacaoContaId", {})
        mapa_meses = mapa_anos.get(ano)
        
        if not mapa_meses:
            return {
                "status": "erro",
                "mensagem": f"Ano '{ano}' não encontrado no arquivo de mapeamentos.",
                "detalhes": {"ano_selecionado": ano}
            }
            
        prestacao_id = mapa_meses.get(mes_ref)
        
        # Se for 0 ou None, avisa o usuário para configurar
        if not prestacao_id:
            return {
                "status": "erro",
                "mensagem": f"ID da Prestação de Contas para {mes_ref}/{ano} não configurado (valor é 0 ou inexistente).",
                "detalhes": {
                     "acao": f"Abra o arquivo Utils/mapeamentos.json e insira o ID correto para ['PrestacaoContaId']['{ano}']['{mes_ref}']."
                }
            }
        
        logging.info(f"PrestacaoContaId encontrado: {prestacao_id} para {mes_ref}/{ano}")
        ...
        # Filtragem de linhas inválidas/vazias
        # Remove linhas que não possuem Nome ou CPF preenchidos (evita erro com linhas em branco no Excel)
        col_nome = cols["Nome"]
        col_cpf = cols["CPF"]
        
        # 1. Remove linhas onde Nome ou CPF são NaN (nulos do pandas)
        df.dropna(subset=[col_nome, col_cpf], how='any', inplace=True)
        
        # 2. Converte para string e remove linhas que ficaram vazias ou são apenas espaços
        df = df[df[col_nome].astype(str).str.strip() != '']
        df = df[df[col_cpf].astype(str).str.strip() != '']
        
        # 3. Remove strings literais "nan" que podem surgir de conversões
        df = df[~df[col_nome].astype(str).str.lower().eq('nan')]
        df = df[~df[col_cpf].astype(str).str.lower().eq('nan')]
        ...
        if mask_cargo0.any():
            orig_cargos = df[cols["CargoId"]].iloc[mask_cargo0.values].unique()
            problemas.append(f"Existem COLABORADORES com Cargo não mapeado: {', '.join(map(str, orig_cargos))}")
            
        mask_linha0 = saida['LinhaServicoId'] == 0
        if mask_linha0.any():
            orig_linhas = df[cols["LinhaServicoId"]].iloc[mask_linha0.values].unique()
            problemas.append(f"Existem COLABORADORES com Linha de Serviço não mapeada: {', '.join(map(str, orig_linhas))}")
        ...
        for i, raw in cpf_series.items():
            if not is_valid_cpf(raw):
                ...
                cpf_inv_values.append(f"'{raw}'")
        
        if mask_cpf_inv:
            problemas.append(f"CPFs inválidos detectados ({len(mask_cpf_inv)}): {', '.join(mask_cpf_inv[:10])}... Valores ex: {', '.join(cpf_inv_values[:5])}")
